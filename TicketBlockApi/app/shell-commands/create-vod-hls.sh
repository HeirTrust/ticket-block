...
for rendition in "${renditions[@]}"; do
  ...
  #echo -e "Wx: ${width}  " # , Hx: {hx}

  #cmd+=" ${static_params} -vf scale=w=${width}:h=${height}:force_original_aspect_ratio=decrease"
  cmd+=" -hls_flags single_file"
  cmd+=" ${static_params} -vf scale=${width}:-2" # :force_original_aspect_ratio=decrease -- auto height ,based on width
  #cmd+=" ${static_params} -vf scale=w=${wx}:h=${hx}:force_original_aspect_ratio=decrease"
