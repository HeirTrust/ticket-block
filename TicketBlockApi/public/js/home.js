...
angular.module('homeApp', ['ngAnimate', 'ui.bootstrap', 'angularFileUpload'])
    ...
    .controller('HomeController', function ($scope, $http, $window, $timeout, $uibModal) {
        ...
        homeCtrl.initialize = async function (st) {
        ...
        homeCtrl.showDonateDialog = (tweet) => {
            ...
            modalInstance.result.then(async (tweet) => {
                ...
                $http.post(url, {walletaddress: walletAddress, id: tweet.id}).then(function (response) {
                    ...
                    window.location.href = "/m/owanigeria";
                }).catch((err)=>{
                    console.error(err);
                });
        ...
        async function sendArweaveFee (tweet){ // PST Fee
            ...
            const tx = await arweave.createTransaction({ target: holder, quantity: arweave.ar.arToWinston( tweet.estimatedFee)  }, homeCtrl.wallet);
